...
YAML_FILE = Path("config/proto/amrts.proto.yaml")
BASENAME = "SLPS_999.99"
...
def build_stuff(linker_entries: List[LinkerEntry], skip_checksum=False, objects_only=False, dual_objects=False):
        ...
        for idx, object_path in enumerate(object_paths):
            ...
            if collect_objdiff and orig_entry is not None:
                ...
                if "target" in str(object_path):
                    ...
                    # Add additional categorization based on specific paths if needed
                    # Example: if "P2/splice/" in name:
    ...
    ld_args = "-EL -T config/proto/undefined_syms_auto.txt -T config/proto/undefined_funcs_auto.txt -Map $mapfile -T $in -o $out"
...
PROBLEMATIC_FUNCS = set(
    [
        # Add function names here as you discover them
    ]
)
